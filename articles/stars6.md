...
Here’s an attempt at the table describing how `raster` functions map to
`stars` functions, discussed in [issue
\#122](https://github.com/r-spatial/stars/issues/122). This table uses
the functionality of the `raster` package as a template; it may be
incomplete, imprecise or plain wrong, so take it with a pinch of salt.
Any comment or correction is hugely appreciated, please contribute!

Some of the functions (`filter`, `slice`, `mutate`, `select`, `pull`, …)
are provided via `dplyr`, which must be loaded. See
[`?stars::dplyr`](../reference/dplyr.md).
...
The format of this table follows the `raster-package` entry in the
raster manual, found at
