...
* `refsys: (character length 1): string describing the reference system; e.g. the PROJ string or `POSIXct` 
* `point: (logical length 1): boolean indicating whether cells/pixels refer to areas/periods, or to points/instances (may be NA)
* `values: NULL, or a vector with coordinate values (numeric, POSIXct, `sfc`), or an object of class `intervals`

An `intervals` object is a list with two vectors, `start` and `end`,
with interval start- and end-values.
...
cropped.  Irregular grids need grid values in `values`; this can be
irregularly _spaced_ coordinate values, or coordinate _intervals_
of irregular width, or even spatial geometries encoded in an `sfc`
vector ("list-column").
