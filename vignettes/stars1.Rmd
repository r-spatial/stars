...
Raster datacubes have at least two spatial dimensions that form the
...
describing dimensins, list extraction (and assignment) works as expected:
...
we now crop the grid to those cells falling in 
```{r}
...
The selection `prec_slice[nc]` essentially calls `st_crop(prec_slice,
nc)` to get a cropped selection.  What happened here is that all
