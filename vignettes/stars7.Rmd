...
Now we need to setup a parallel backend by defining the number of available cores (or by detecting them automatically using `detectCores()`) in `makeCluster()` and then registering the cluster using the `registerDoParallel()` function.
Due to hardware limitations, we use only two cores in this example, but in reality this number should be higher.
This can be accomplished, for instance, with
...
Next, we make the prediction using the previously developed model and save the result in the temporary directory.
...
The best way to do this is by using a virtual file (`buildvrt`) and converting directly from GDAL (`translate`) to a single geotiff.
Both of these tools are available in `sf::gdal_utils()`.

```{r eval=EVAL}
vrt = tempfile(fileext = ".vrt")
gdal_utils(util = "buildvrt", unlist(tifs), vrt)
dest = file.path(tempdir(), "predict.tif")
gdal_utils(util = "translate", vrt, dest)
read_stars(dest)
```
