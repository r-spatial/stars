...
```{r}
(l7.spl = split(l7, "band"))
as.data.frame(l7.spl) %>% head()
```
...
Another example would be to calibrate a model on each pixel time series, and output one or more model coefficients for each pixel; this is shown here.
...
```{r}
plot(merge(out), hook = function() plot(m, border = 'orange', add = TRUE))
```
This suggests that PC1 picks up the difference between clouds and non-clouds, and PC2 the difference between sea and land areas.
...
rf = randomForest(use~B4+B3+B2+B8, train)
pr = predict(r, rf)
plot(pr, key.width = lcm(5), reset = FALSE)
# add country outline
plot(m, add = TRUE)
```